...
class DataIngestionJob:
    """
    A data ingestion job template with enhanced validations and production best practices.
    SparkSession is provided externally; AWS clients are initialized inside.

    The DataFrame is first written to a temporary S3 location (bucket_temp + path_temp).
    If validation succeeds, the DataFrame is then written to the final ingestion location (bucket_ingest + path_ingest).
    """
    def __init__(self, config: dict, spark: SparkSession):
        """Initialize job with config and SparkSession; create AWS clients internally."""
        self.config = config
        self.logger = logging.getLogger(self.__class__.__name__)
        logging.basicConfig(level=logging.INFO)
        self.spark = spark

        # Initialize AWS clients using config
        aws_conf = self.config.get("aws", {})
        region = aws_conf.get("region_name")
        clients = aws_conf.get("clients", ["s3"])
        self.s3_client = boto3.client("s3", region_name=region) if "s3" in clients else None
        self.sns_client = boto3.client("sns", region_name=region) if "sns" in clients and self.config.get("sns_topic_arn") else None
        self.secrets_client = boto3.client("secretsmanager", region_name=region) if self.config.get("secrets_manager_name") else None

        self._validate_config()
        self._init_connections()

    def get_secrets(self, secret_name: str) -> dict:
        """Placeholder to retrieve secrets from AWS Secrets Manager."""
        # TODO: implement actual secrets retrieval logic
        # Example:
    ...
    def _validate_config(self):
        """Ensure required config keys and subkeys exist."""
        self.logger.info("Validating configuration keys")
        required_keys = ["source", "target", "temp", "ingest_date"]
        for key in required_keys:
            if key not in self.config:
                raise ValueError(f"Missing required config key: {key}")
        ...
        # Validate temp subkeys
        temp = self.config.get("temp", {})
        for sub in ["bucket", "path"]:
            if sub not in temp or not temp.get(sub):
                raise ValueError(f"Missing or empty temp parameter: {sub}")
    ...
    def _init_connections(self):
        ...
        if self.config.get("secrets_manager_name") and self.secrets_client:
            try:
                secret_data = self.get_secrets(self.config["secrets_manager_name"])
                self.config["source"]["user"] = secret_data.get("user")
                self.config["source"]["password"] = secret_data.get("password")
                self.logger.info("Fetched source credentials from Secrets Manager")
            except Exception as e:
        ...
        tgt = self.config["target"]
        self.bucket_ingest = tgt.get("bucket")
        self.path_ingest = tgt.get("path", "").rstrip("/")
        self.logger.info(f"Configured ingest S3 target: s3://{self.bucket_ingest}/{self.path_ingest}")

        # Temporary write location
        temp = self.config["temp"]
        self.bucket_temp = temp.get("bucket")
        self.path_temp = temp.get("path", "").rstrip("/")
        self.logger.info(f"Configured temp S3 target: s3://{self.bucket_temp}/{self.path_temp}")
    ...
    def write_data(self, df: DataFrame):
        """Write DataFrame first to temp, then to ingest if validation succeeds."""
        ingest_date = self.config.get("ingest_date")
        partition_key = self.config.get("partition_key", "ingest_date")
        overwrite = self.config.get("overwrite_partition", False)

        # 1. Write to temp location
        temp_base = f"s3://{self.bucket_temp}/{self.path_temp}"
        temp_path = f"{temp_base}/{partition_key}={ingest_date}"
        self.logger.info(f"Writing data to temp S3 path: {temp_path}")
        try:
            start_temp = time.time()
            df.write.mode("overwrite").parquet(temp_path)
            self.logger.info(f"Temp write completed in {time.time() - start_temp:.2f} seconds")
        except Exception as e:
            self.logger.error(f"Parquet write to temp failed: {e}", exc_info=True)
            raise

        # 2. Validate data (using DataFrame in memory)
        if not self.validate_data(df):
            raise ValueError("Data validation failed; aborting final write.")

        # 3. Write to final ingest location
        target_base = f"s3://{self.bucket_ingest}/{self.path_ingest}"
        target_path = f"{target_base}/{partition_key}={ingest_date}"
        write_mode = "overwrite" if overwrite else "append"
        self.logger.info(f"Writing data to ingest S3 path: {target_path} with mode {write_mode}")
        try:
            start_ingest = time.time()
            df.write.mode(write_mode).parquet(target_path)
            self.logger.info(f"Ingest write completed in {time.time() - start_ingest:.2f} seconds")
        except Exception as e:
            self.logger.error(f"Parquet write to ingest failed: {e}", exc_info=True)
            raise

        # 4. Write _SUCCESS marker to ingest
        if self.s3_client:
            marker_key = f"{self.path_ingest}/{partition_key}={ingest_date}/_SUCCESS"
            try:
                self.s3_client.put_object(Bucket=self.bucket_ingest, Key=marker_key, Body=b"")
                self.logger.info(f"_SUCCESS marker written to s3://{self.bucket_ingest}/{marker_key}")
            except Exception as e:
                self.logger.warning(f"Failed to write _SUCCESS marker: {e}", exc_info=True)
        else:
            self.logger.warning("No S3 client available; skipping _SUCCESS marker.")
    ...
    def run(self):
        """Execute the ingestion: read → transform → write with metrics."""
        job_start = time.time()
        self.logger.info("Starting ingestion job")
        try:
            df_raw = self.read_data()
...
# -----------------------------------------------------------------------------
# Example usage without a CLI (Spark provided externally)
# -----------------------------------------------------------------------------
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Enhanced Data Ingestion Job")
    parser.add_argument("--config", required=True, help="Path to JSON config file for the job")
    ...
    spark = SparkSession.builder.appName(config.get("job_name", "IngestionJob")).getOrCreate()
    job = DataIngestionJob(config, spark=spark)
    job.run()
...
#     "url": "jdbc:postgresql://hostname:5432/dbname",
#     "table": "public.sales",
#     "user": "",  # overridden by secrets if configured
#     "password": "",
#     "driver": "org.postgresql.Driver"
#   },
#   "target": {
